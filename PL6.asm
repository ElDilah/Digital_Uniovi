...
;INICIALIZACION 
    ...
    CLRF CENTESIMAS ;EL RESTO DE VALORES A 0
    CLRF ESTADO ;PONEMOS ESTADO INICIAL RESETEO
    CLRF MINUTOS ;PONER A 0 MINUTOS
...
BUCLE 
    MOVF MINUTOS,F
    BTFSC STATUS,Z 		;REVISAMOS STATUS SI HAY 0 MINUTOS O NO 
    GOTO SIN_MIN ;NO
    GOTO CON_MIN ;SI 
...
SIN_MIN movlw 	b'11111101'	;Activamos el display de la izquierda
    ...
    movlw 	b'11110111'     ;Activamos el display de la derecha con
    movwf 	PORTA          	;el bit RA3 a 0 y el resto a 1
    movf 	CENTESIMAS,W     	;CARGAR LAS DECENAS DE SEGUNDO
    call  	TABLA_U     	;y se llama al subprograma que controla los 7 diodos led
    MOVWF PORTD ;VALOR
    CALL ESPERA
    GOTO SELECCION
;-----------------------------1 PARTE MOSTRAR VALOR ---------------------------------
SELECCION MOVF ESTADO,W
    ADDWF PCL,F
...
ESTADO_1 
    ...
    bcf	T1CON,TMR1ON	;Si estamos en el estado 1, TMR1 parado 
...
ESTADO_2 
    ...
    BCF INTCON,INTF ;QUITAR EL FLAG
    GOTO BUCLE  
...
COMP_UNI MOVLW 0x0A  ;CARGAMOS EN W EL VALOR 10
    ...
    CLRF UNIDADES
    RETURN
COMP_DEC MOVLW 0x06  ;CARGAMOS EN W EL VALOR 10
    XORWF DECENAS,W ;COMPROBAMOS EL VALOR Y GUARDAMOS EN W
    BTFSS STATUS,Z ;COMPROBAMOS ESTADO DE Z
    RETURN  ; 0- NO NOS HEMOS PASADO DE 10
    INCF MINUTOS ; AÑADIMOS 1 A LAS DECENAS EN CASO DE PASARNOS 
        CLRF DECENAS
    MOVLW 0x06  ;CARGAMOS EN W EL VALOR 10
    XORWF MINUTOS,W ;COMPROBAMOS EL VALOR Y GUARDAMOS EN W
    BTFSS STATUS,Z ;COMPROBAMOS ESTADO DE Z
      RETURN
    CLRF MINUTOS
    MOVLW D'5'
...
COMP_CENT
   ...
   movwf CENTESIMAS
    ...
    BTFSS STATUS,Z ;ESTA LA CUENTA A 0 ?
    RETURN ;NO RECUPERAR 
    DECF DECENAS ;SI PUES RESTAMOS 1 SEGUNDO 
   movlw d'9'
   movwf UNIDADES
    MOVLW 0xFF ;PONER EL VALOR FF PARA COMPROBAR SI DESBORDO MILESIMAS POR DEBAJO
    SUBWF DECENAS,W ; 
